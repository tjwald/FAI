...
- **Testing Style Guide**:
    ...
    - **Mocks**: When testing decorators or schedulers, verify that exact input ranges reach the inner step and preallocated slices receive the expected values.
...
## Critical Patterns
- **Finite Steps**: ML tasks implement return-value `IStep<TInput, TOutput>`. Implement `IPreallocatingStep<TInput, TOutput>` only when output storage can be derived synchronously from metadata and execution can write a compatible destination.
